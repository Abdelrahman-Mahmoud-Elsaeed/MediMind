...
async registerProvider(userData) {
  const { credentials, email, phone, nationalNumber, licenseNumber, role } = userData;
  let account;
  let profile;

  // Resolve primary email/phone from credentials first, fallback to root body fields
  ...
  await session.withTransaction(async () => {
    ...
    account = new Account({
      ...
      role: role || "PROFESSIONAL_CAREGIVER",
      isActive: false,
      sessions: [],
    });
