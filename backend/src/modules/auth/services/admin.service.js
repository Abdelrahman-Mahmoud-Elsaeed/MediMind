...
class AdminService {
    ...
    if (requiredPermission) {
      const has =
        ...
        (adminProfile.permissions.includes("*") ||
          adminProfile.permissions.includes(requiredPermission));
      if (!has)
  ...
  async registerProfessional(adminAccountId, userData) {
    ...
    await account.save();
    const profile = new ProfessionalCaregiver({
    ...
    await profile.save();
    logger.info(
      `[AdminAction] Admin ${adminAccountId} created PROFESSIONAL_CAREGIVER ${account._id}`,
    );
    return new ServiceResponse({
      status: "CREATED",
      en: "Professional caregiver created.",
      ar: "تم إنشاء مقدم الرعاية.",
      data: {
        accountId: account._id,
  ...
  async registerProvider(adminAccountId, userData) {
    ...
    if (!["DOCTOR", "PHARMACIST"].includes(role))
      throw new AppError("Invalid provider role", 400, "INVALID_ROLE", {
        en: "Role must be DOCTOR or PHARMACIST.",
        ar: "الدور يجب أن يكون DOCTOR أو PHARMACIST.",
      });
    await _verifyUniqueness({ email: userData.email, phone: userData.phone });
    const account = new Account({
      email: userData.email,
    ...
    await profile.save();
    logger.info(
      `[AdminAction] Admin ${adminAccountId} created ${role} ${account._id}`,
    );
    return new ServiceResponse({
      status: "CREATED",
      en: `${role} account created.`,
      ar: `تم إنشاء حساب ${role}.`,
      data: {
        accountId: account._id,
        profileId: profile._id,
        role: account.role,
  ...
  _buildProviderProfileData(role, userData) {
    ...
    if (role === "DOCTOR") {
      const data = {
        ...
        syndicateId: userData.syndicateId,
        clinicName: userData.clinicName,
        clinicAddress: userData.clinicAddress || {},
      };
      if (
        userData.coordinates &&
        Array.isArray(userData.coordinates) &&
        userData.coordinates.length === 2
      )
        data.location = { type: "Point", coordinates: userData.coordinates };
      return data;
    }
    if (role === "PHARMACIST") {
      const data = {
        ...
        ownerName:
          userData.ownerName || `${userData.firstName} ${userData.lastName}`,
        licenseNumber: userData.licenseNumber,
  ...
  async verifyDoctor(adminAccountId, doctorProfileId) {
    const adminProfile = await this._requireAdmin(
      adminAccountId,
      "users.verify",
    );
    ...
    logger.info(
      `[AdminAction] Admin ${adminAccountId} verified Doctor ${doctor._id}`,
    );
    return new ServiceResponse({
      status: "SUCCESS",
      en: "Doctor verified.",
      ar: "تم التحقق من الطبيب.",
      data: {
  ...
  async verifyPharmacist(adminAccountId, pharmacistProfileId) {
    ...
    if (!pharmacist)
      throw new AppError("Pharmacist not found", 404, "PHARMACIST_NOT_FOUND", {
        en: "Pharmacist not found.",
        ar: "الصيدلي غير موجود.",
      });
    if (pharmacist.subscription && pharmacist.subscription.status === "active")
      throw new AppError("Already verified", 400, "ALREADY_VERIFIED", {
        en: "Already verified.",
        ar: "تم التحقق بالفعل.",
      });
    ...
    logger.info(
      `[AdminAction] Admin ${adminAccountId} verified Pharmacist ${pharmacist._id}`,
    );
    return new ServiceResponse({
      status: "SUCCESS",
      en: "Pharmacist verified.",
      ar: "تم التحقق من الصيدلي.",
      data: {
        profileId: pharmacist._id,
        accountId: pharmacist.accountId,
        subscriptionStatus: pharmacist.subscription.status,
        verifiedBy: adminProfile._id,
      },
    });
  }
  ...
  async updateAccountStatus(
    ...
    if (targetAccountId.toString() === adminAccountId.toString())
      throw new AppError(
        "Cannot modify own account",
        400,
        "INVALID_OPERATION",
        { en: "Cannot modify own account.", ar: "لا يمكن تعديل حسابك." },
      );
    ...
    if (account.role === "ADMIN" && adminProfile.adminType !== "super_admin")
      throw new AppError("Cannot modify admin accounts", 403, "FORBIDDEN", {
        en: "Only super admins can modify admins.",
        ar: "فقط المسؤول الأعلى يمكنه تعديل المسؤولين.",
      });
    ...
    await account.save();
    logger.info(
      `[AdminAction] Admin ${adminAccountId} set account ${targetAccountId} isActive=${isActive}`,
    );
    return new ServiceResponse({
      status: "SUCCESS",
      en: `Account ${isActive ? "activated" : "deactivated"}.`,
      ar: `تم ${isActive ? "تفعيل" : "تعطيل"} الحساب.`,
      data: {
        accountId: account._id,
        role: account.role,
        previousStatus,
