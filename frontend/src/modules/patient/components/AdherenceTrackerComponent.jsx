...
export default function AdherenceTrackerComponent() {
    ...
    const { currentDate, selectedDate, setSelectedDate, doses, loading, error, takenCount, missedCount, skippedCount, pendingCount, adherenceRate, confirmDose, skipDose, changeMonth } = useAdherenceTracker();
    ...
    const daysOfWeek = locale === "ar"
        ? ["ح", "ن", "ث", "ر", "خ", "ج", "س"]
        : ["S", "M", "T", "W", "T", "F", "S"];
    return (<MainLayout activePath="/adherence">
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <section className="bg-surface-container border border-outline-variant/10 rounded-2xl p-6 flex flex-col items-center shadow-sm">
          <div className="relative w-40 h-40 flex items-center justify-center mb-6">
            <svg className="w-full h-full transform -rotate-90">
              ...
              <circle cx="80" cy="80" r="70" className="stroke-primary fill-none transition-all duration-500" strokeWidth="12" strokeDasharray={2 * Math.PI * 70} strokeDashoffset={2 * Math.PI * 70 * (1 - adherenceRate / 100)}/>
            </svg>
        ...
        <section className="bg-surface-container border border-outline-variant/10 p-5 rounded-2xl">
          <div className="grid grid-cols-7 gap-2 mb-4">
            {daysOfWeek.map((day, idx) => (<div key={idx} className="text-center font-label-sm text-xs text-on-surface-variant/50 font-bold">
                {day}
              </div>))}
          </div>
          <div className="grid grid-cols-7 gap-y-3">
            ...
            return (<div key={idx} onClick={() => setSelectedDate(dateObj)} className={`flex flex-col items-center justify-center h-10 rounded-xl relative ${isSelected
        ...
        <section className="space-y-4">
          ...
          {loading ? (<div className="text-center py-6 text-slate-500">Loading daily logs...</div>) : error ? (<div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl text-center">
              {error}
            </div>) : doses.length > 0 ? (<div className="flex flex-col gap-3">
              {doses.map((log) => {
                ...
                return (<div key={log.doseEventId} className={`bg-surface-container border border-outline-variant/10 p-4 rounded-2xl flex flex-col gap-3 border-l-4 ${statusColor} transition-all`}>
                    ...
                    {log.status === "PENDING" && (<div className="flex gap-2 pt-2 border-t border-outline-variant/5">
                        <button onClick={() => confirmDose(log.doseEventId)} className="flex-1 py-2 bg-primary text-on-primary text-xs font-bold rounded-lg hover:brightness-110 active:scale-95 transition-all">
                          {t("patient.adherence.confirmTaken")}
                        </button>
                        <button onClick={() => skipDose(log.doseEventId)} className="flex-1 py-2 border border-outline-variant text-on-surface text-xs font-bold rounded-lg hover:bg-surface-variant/20 active:scale-95 transition-all">
                          {t("patient.adherence.skipDose")}
                        </button>
                      </div>)}
                  </div>);
            })}
            </div>) : (<div className="bg-surface-container border border-outline-variant/10 rounded-2xl p-8 text-center text-slate-500">
