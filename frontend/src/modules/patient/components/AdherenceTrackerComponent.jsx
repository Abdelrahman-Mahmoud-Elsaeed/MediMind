...
export default function AdherenceTrackerComponent() {
  ...
  const [timeFrame, setTimeFrame] = useState("30days"); // '30days' | '90days' | 'yearly'
  ...
  // Mock adherence heatmap days (30 days grid)
  const heatmapDays = useMemo(() => {
    const days = [];
    for (let i = 1; i <= 30; i++) {
      let status = "perfect"; // 'perfect' | 'partial' | 'missed'
      if (i === 5 || i === 19) status = "missed";
      else if (i === 3 || i === 12 || i === 24) status = "partial";
  ...
  const medicationPerformance = useMemo(() => {
    if (medications && medications.length > 0) {
      return medications.map((med, idx) => ({
        ...
        dosage: med.dosage || "500mg • Twice Daily",
        percentage: 95 - idx * 6,
        bars: [80 + idx * 2, 90, 85, 95 + idx, 98],
        subtitle:
          idx === 0
            ? isAr ? "التزام شبه كامل هذا الشهر" : "Near perfect compliance this month"
            : idx === 1
            ? isAr ? "جرعة واحدة فائتة يوم الأحد" : "1 missed dose on Sunday"
            : isAr ? "اتجاه صاعد (+٥٪)" : "Trending upwards (+5%)",
      }));
    }
    ...
    return [
      {
        ...
        percentage: 98,
        bars: [90, 95, 92, 98, 100],
        subtitle: isAr ? "التزام شبه كامل هذا الشهر" : "Near perfect compliance this month",
      },
      {
        id: "2",
        name: "Lisinopril",
        ...
        percentage: 92,
        bars: [85, 90, 88, 92, 92],
        subtitle: isAr ? "جرعة واحدة فائتة يوم الأحد" : "1 missed dose on Sunday",
      },
      {
        id: "3",
        name: "Atorvastatin",
        ...
        percentage: 89,
        bars: [75, 80, 84, 87, 89],
        subtitle: isAr ? "اتجاه صاعد (+٥٪)" : "Trending upwards (+5%)",
      },
    ];
  }, [medications, isAr]);
  ...
  const activityLogs = useMemo(() => {
    if (doses && doses.length > 0) {
      return doses.map((d, i) => {
        ...
        const timeFormatted = new Date(d.scheduledFor || Date.now()).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        });

        return {
          id: d.doseEventId || String(i),
          medication: d.medicationName || "Medication",
          timeFrameLabel: i === 0 ? (isAr ? "اليوم" : "Today") : (isAr ? "أمس" : "Yesterday"),
          scheduled: timeFormatted,
          takenAt: isTaken ? timeFormatted : isSkipped ? (isAr ? "تم التخطي" : "Skipped") : "—",
          status: isTaken ? "On Time" : isSkipped ? "Skipped" : isMissed ? "Missed" : "On Time",
          rawStatus: d.status,
        };
      });
    }
    ...
    return [
      ...
      },
      {
        ...
        timeFrameLabel: isAr ? "أمس" : "Yesterday",
        scheduled: "09:00 PM",
        takenAt: "09:45 PM",
        status: "Late",
      },
      ...
      },
      {
        id: "a5",
        medication: "Lisinopril 5mg",
        timeFrameLabel: isAr ? "منذ يومين" : "2 days ago",
        scheduled: "08:00 AM",
        takenAt: "—",
        status: "Missed",
      },
      {
        id: "a6",
        medication: "Atorvastatin 10mg",
        timeFrameLabel: isAr ? "منذ ٣ أيام" : "3 days ago",
        scheduled: "09:00 PM",
        takenAt: "09:05 PM",
        status: "On Time",
      },
      ...
      <div className="max-w-[1400px] mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xs hover:shadow-md transition-all">
            ...
            <div className="text-4xl font-black text-[#0b1c30] dark:text-slate-100 tracking-tight">94.2%</div>
            <div className="flex items-center gap-1 mt-2 text-xs font-bold text-teal-600 dark:text-teal-400">
              <TrendingUp className="w-3.5 h-3.5" />
              <span>{isAr ? "+٢٫٤٪ عن الشهر الماضي" : "+2.4% from last month"}</span>
            </div>
          </Card>
          ...
          <Card className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xs hover:shadow-md transition-all">
            ...
            <div className="text-4xl font-black text-[#0b1c30] dark:text-slate-100 tracking-tight">
              {isAr ? "١٨ يوماً" : "18 Days"}
            </div>
            <div className="text-xs font-bold text-slate-500 dark:text-slate-400 mt-2">
              {isAr ? "أفضل رقم قياسي: ٢٢ يوماً" : "Personal Best: 22 days"}
            </div>
          </Card>
          ...
          <Card className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xs hover:shadow-md transition-all">
            ...
            <div className="text-4xl font-black text-[#0b1c30] dark:text-slate-100 tracking-tight">24/30</div>
            <div className="mt-3">
              <ProgressBar value={80} className="h-2.5 bg-slate-100 dark:bg-slate-800" />
            </div>
          </Card>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-teal-100 dark:border-slate-700/60 flex items-start gap-3 shadow-2xs">
                ...
                <p className="text-xs text-slate-700 dark:text-slate-300 font-medium leading-relaxed">
                  {isAr ? (
                    <>أنت أكثر التزاماً مع <strong>جرعات ٨:٠٠ صباحاً</strong>. يُنصح بربط العادات الأخرى بهذا التوقيت.</>
                  ) : (
                    <>You are most consistent with your <strong>8:00 AM doses</strong>. Consider linking other habits to this timeframe.</>
                  )}
                </p>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-teal-100 dark:border-slate-700/60 flex items-start gap-3 shadow-2xs">
                ...
                <p className="text-xs text-slate-700 dark:text-slate-300 font-medium leading-relaxed">
                  {isAr ? (
                    <>تحسن التزامك في عطلة نهاية الأسبوع <strong>بنسبة ١٥٪ هذا الشهر</strong>. إعدادات التنبيهات الجديدة تعمل بفعالية.</>
                  ) : (
                    <>Weekend adherence has <strong>improved by 15% this month</strong>. Your new notification settings are working well.</>
                  )}
                </p>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-teal-100 dark:border-slate-700/60 flex items-start gap-3 shadow-2xs">
                ...
                <p className="text-xs text-slate-700 dark:text-slate-300 font-medium leading-relaxed">
                  {isAr ? (
                    <>أطول سلسلة متواصلة مسجلة هذا العام: <strong>٢٢ يوماً</strong>. يفصلك ٤ أيام فقط عن تحطيم رقمك القياسي!</>
                  ) : (
                    <>Longest streak recorded this year: <strong>22 Days</strong>. You are only 4 days away from beating it!</>
                  )}
                </p>
              </div>
            </div>
