...
export default function PatientProfileComponent() {
    ...
    const { profile, caregiver, loading, error, isEditing, setIsEditing, saving, validationError, phone, dob, setDob, bloodType, setBloodType, ecName, setEcName, ecPhone, setEcPhone, updateProfile } = usePatientProfile();
    ...
    return (<MainLayout activePath="/profile">
      <div className="max-w-[1400px] mx-auto space-y-8">
        {loading ? (<div className="text-center py-12 text-slate-500">Loading profile...</div>) : error ? (<div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl text-center">
            {error}
            ...
            <section className="lg:col-span-4 space-y-4">
              <div className="bg-surface-container border border-outline-variant/10 rounded-2xl p-6 flex flex-col items-center text-center">
                <div className="relative mb-4">
                  ...
                  <button onClick={() => setIsEditing(true)} className="absolute bottom-0 right-0 bg-primary text-on-primary p-2 rounded-full shadow-lg hover:scale-105 active:scale-95 transition-transform flex items-center justify-center">
              ...
              <div className="bg-surface-container border border-outline-variant/10 rounded-2xl p-6 border-l-4 border-l-secondary">
                ...
                {caregiver ? (<div className="flex items-center gap-4 bg-background/50 p-4 rounded-xl border border-outline-variant/5">
                  ...
                  </div>) : (<div className="text-center py-2 text-on-surface-variant/60 text-xs">
            ...
            <section className="lg:col-span-8 space-y-4">
              <div className="bg-surface-container border border-outline-variant/10 rounded-2xl overflow-hidden divide-y divide-outline-variant/10">
                {/* Row 1: Personal Info */}
                <button onClick={() => setIsEditing(true)} className="w-full flex items-center p-5 gap-4 hover:bg-surface-container-high transition-colors text-left group">
      ...
      {isEditing && (<div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-surface-container border border-outline-variant/10 p-6 rounded-2xl max-w-md w-full space-y-4">
            ...
            {validationError && (<div className="bg-red-500/10 border border-red-500/20 text-red-400 p-3 rounded-xl text-center text-xs font-bold">
                {validationError}
              </div>)}
            ...
            <form onSubmit={updateProfile} className="space-y-4">
              <div>
                ...
                <select value={bloodType} onChange={(e) => setBloodType(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-2.5 outline-none text-on-surface focus:ring-2 focus:ring-primary">
              ...
              <div>
                ...
                <input type="date" value={dob} max={(() => {
                const d = new Date();
                d.setFullYear(d.getFullYear() - 12);
                return d.toISOString().split('T')[0];
            })()} onChange={(e) => setDob(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-2.5 outline-none text-on-surface focus:ring-2 focus:ring-primary"/>
              </div>

              <div className="border-t border-outline-variant/10 pt-3">
                <p className="text-xs font-bold text-on-surface-variant mb-2">Emergency Contact</p>
                <div className="space-y-2">
                  <input type="text" placeholder="Name" value={ecName} onChange={(e) => setEcName(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-2.5 outline-none text-on-surface focus:ring-2 focus:ring-primary"/>
                  <input type="text" placeholder="Phone" value={ecPhone} onChange={(e) => setEcPhone(e.target.value)} className="w-full bg-surface-container-low border border-outline-variant/20 rounded-xl px-4 py-2.5 outline-none text-on-surface focus:ring-2 focus:ring-primary"/>
                </div>
              </div>

              <div className="flex gap-3 pt-4 border-t border-outline-variant/10">
                <button type="button" onClick={() => setIsEditing(false)} className="flex-1 py-2.5 border border-outline-variant text-on-surface font-bold rounded-xl hover:bg-surface-variant/25 transition-all text-xs">
                  {t("patient.profile.cancel")}
                </button>
                <button type="submit" disabled={saving} className="flex-1 py-2.5 bg-primary text-on-primary font-bold rounded-xl shadow-lg hover:brightness-110 active:scale-95 transition-all text-xs disabled:opacity-50">
