...
const BottleSpoonIcon: React.FC<{ className?: string }> = ({ className = 'w-6 h-6' }) => (
    ...
    >
      <div>
        ...
        <div className="flex justify-between items-start mb-4">
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${iconBgMap[medication.iconType]}`}
          >
      ...
      <MedicationActions
        ...
        onSchedule={() => onSchedule?.(medication.id)}
        onRefill={() => onRefill?.(medication.id)}
        isUrgent={isUrgent}
      />
    </motion.div>
  );
