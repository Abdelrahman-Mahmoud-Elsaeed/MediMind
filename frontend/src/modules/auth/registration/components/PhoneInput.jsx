...
export function PhoneInput({ id = "loginInput", value, onChange, onBlur, error, touched, isPhoneInput, countrySelectorProps, placeholder, label, required = true, disabled = false, showAsterisk = false, }) {
    ...
    return (<div className="w-full">
      ...
      <div className="relative flex items-center">
        <span className={`material-symbols-outlined absolute ${isRtl ? "end-4" : "start-4"} top-1/2 -translate-y-1/2 text-on-surface-variant pointer-events-none z-10 !text-[22px]`}>
          {isPhoneInput ? "call" : "mail"}
        </span>

        <input id={id} type="text" required={required} value={value} onChange={onChange} onBlur={onBlur} disabled={disabled} placeholder={fieldPlaceholder} dir={isRtl ? "rtl" : "ltr"} className={`w-full h-[58px] font-['Inter'] text-base md:text-lg text-on-surface bg-surface-container-lowest border ${stateBorderClass} focus:outline-none placeholder:text-on-surface-variant/60 dark:placeholder:text-on-surface-variant/70 transition-all rounded-[16px] shadow-sm disabled:opacity-50 disabled:bg-surface-container-low ${isPhoneInput ? "ps-[48px] pe-[145px]" : "ps-[48px] pe-[48px]"}`}/>
