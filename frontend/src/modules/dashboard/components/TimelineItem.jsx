...
export const TimelineItem = ({ item, isFirst = false, isLast = false, onMarkAsTaken, onSnooze, }) => {
    ...
    return (<div className="relative flex items-stretch gap-4 sm:gap-6 group">
      ...
      <div className="w-10 shrink-0 relative flex flex-col items-center select-none">
        ...
        {!isFirst && (<div className={`absolute top-0 h-[18px] w-[2px] ${upperLineColor} pointer-events-none transition-colors duration-300 z-0`}/>)}

        {/* Lower Vertical Connector Line Segment */}
        {!isLast && (<div className={`absolute top-[46px] bottom-0 w-[2px] ${lowerLineColor} pointer-events-none transition-colors duration-300 z-0`}/>)}

        {/* Node Circle Indicator - Center aligned at top-[18px] (y-center = 32px) */}
        <div className="absolute top-[18px] z-10">
          {isCompleted ? (<motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }} className="w-7 h-7 rounded-full bg-primary text-on-primary flex items-center justify-center shadow-xs">
              <Check className="w-4 h-4 stroke-[3]"/>
            </motion.div>) : isDue ? (<div className="relative w-7 h-7 flex items-center justify-center">
            ...
            </div>) : (<div className="w-7 h-7 rounded-full border-2 border-outline-variant/60 bg-surface-container-lowest flex items-center justify-center shadow-2xs">
      ...
      <div className="flex-1 pb-6">
        <motion.div whileHover={{ y: -1 }} transition={{ duration: 0.2 }} className={`relative p-5 rounded-2xl border transition-all duration-300 ${isDue
          ...
          {isDue && (<div className={`absolute top-[26px] ${isRtl
                ? '-right-2 border-y-[6px] border-y-transparent border-l-[8px] border-l-primary-container/20 dark:border-l-primary-container/40'
                : '-left-2 border-y-[6px] border-y-transparent border-r-[8px] border-r-primary-container/20 dark:border-r-primary-container/40'} w-0 h-0 pointer-events-none`}/>)}
          ...
          <div className="flex justify-between items-start mb-1.5 gap-4">
            ...
            <div className="flex flex-col items-end gap-1.5 shrink-0">
              ...
              {isDue && (<AppBadge variant="default" className="shadow-2xs">
          ...
          {isDue && (<div className="flex flex-wrap items-center gap-3 pt-3 mt-3 border-t border-outline-variant/20">
              ...
              <AppButton type="button" variant="outline" size="sm" onClick={() => onSnooze?.(targetId)} leftIcon={<Clock className="w-3.5 h-3.5 text-primary"/>}>
