...
import { useSendOtpMutation, useVerifyOtpMutation } from "@/modules/auth/hooks/useAuthQueries";
...
export default function VerifyComponent() {
  ...
  const sendOtpCode = async () => {
    ...
    try {
      ...
      await sendOtpMutation.mutateAsync({ target, type });
  ...
  const handleVerify = async (e) => {
    ...
    try {
      const type = activeUser.email ? "EMAIL" : (activeUser.phone ? "PHONE" : "EMAIL");
      await verifyOtpMutation.mutateAsync({ type, code });
            ...
            <div className="text-center pt-2">
              <p className="text-sm text-on-surface-variant font-medium">
                ...
                <button
                  type="button"
                  onClick={sendOtpCode}
                  disabled={isResendDisabled || sendOtpMutation.isPending}
